/*
// Hojuix Intel HDA Driver
// Based heavily off of BleskOS's HDA driver
*/

/*
// Notes:
// ONLY supports Codec 0
...
uint32_t aonn = 0; // Audio output node number
...
void hda_init(uintptr_t bar_tbl_addr) {
    ...
    // Disable interrupts
    hda_mmio_out16(hda_base + 0x5A, 0xFFFF); // HACK: Set to 0xFFFF for it to work - https://forum.osdev.org/viewtopic.php?t=56072
    hda_rirb_pointer = 1;
    ...
    uint32_t codec_id = hda_send_verb(0, 0, 0xF00, 0);
    if (codec_id == 0) {
        printf("[HDA] Codec 0 does not exist.\n");
        return;
    }
    hda_init_codec(0);
...
void hda_init_codec(uint32_t codec_number) {
    ...
    if (codec_id == 0x00000000) {
        printf("[HDA] Codec %d does not exist.\n", codec_number);
    }
...
uint8_t hda_is_headphone_connected(uint32_t codec, uint32_t pin_headphone_node_number) {
    if (pin_headphone_node_number != 0 && (hda_send_verb(codec, pin_headphone_node_number, 0xF09, 0x00) & 0x80000000) == 0x80000000) {
        return 1;
    } else {
        return 0;
    }
...
void hda_init_afg(uint32_t codec_number, uint32_t afg_node_number) {
    ...
    for(uint32_t node = ((subordinate_node_count_res>>16) & 0xFF), last_node = (node+(subordinate_node_count_res & 0xFF)), type_of_node = 0; node < last_node; node++) {
        ...
        if (type_of_node == HDA_WIDGET_AUDIO_OUTPUT) {
            ...
            if (type_of_node == HDA_PIN_LINE_OUT) {
                ...
                } // First speaker is default

                // Find if there is a device connected to this pin
                if ((hda_send_verb(codec_number, node, 0xF00, 0x09) & 0x4) == 0x4) {
                    // Find if it's a jack or fixed device
                    if ((hda_send_verb(codec_number, node, 0xF1C, 0x00) >> 30) != 0x1) {
                        // There is a connected device
                        printf("Connected device found on speaker\n");

                        if (pin_speaker_node_number == 0) {
                            pin_speaker_node_number = 0;
                        }
                    } else {
                        // nothing connected
                    }
                } else {
                    // not a jack or fixed device
                }
            
            
            
            } else if (type_of_node == HDA_PIN_HEADPHONE_OUT) {
                printf("[HDA] Found headphone out.\n");
    ...
    uint32_t pin_output_node_number = 0;
    ...
    if (pin_speaker_default_node_number != 0) {
        ...
        if (pin_speaker_node_number != 0) {
            // Speaker output
            hda_init_output_pin(codec_number, pin_speaker_node_number);
            pin_output_node_number = pin_speaker_node_number;
        } else {
            // Default speaker output
            hda_init_output_pin(codec_number, pin_speaker_default_node_number);
            pin_output_node_number = pin_speaker_default_node_number;
        }

        if (pin_headphone_node_number != 0) {
            // Headphone output
            hda_init_output_pin(codec_number, pin_headphone_node_number);
        }

        // Find headphone connection status
        if (hda_is_headphone_connected(codec_number, pin_headphone_node_number)) {
            hda_send_verb(codec_number, pin_output_node_number, 0x707, (hda_send_verb(codec_number, pin_output_node_number, 0xF07, 0x00) & ~0x40));
        }
    } else if (pin_headphone_node_number != 0) {
        // Codec does not have a pseaker, but only headphone output
        hda_init_output_pin(codec_number, pin_headphone_node_number);
    } else if (pin_alternative_output_node_number != 0) {
        // Codec only has alternative output
        hda_init_output_pin(codec_number, pin_alternative_output_node_number);
    } else {
        // what the fuck is going on here, there is no output pin
    }
    ...
    if (pin_headphone_node_number != 0) {
        ...
        if (hda_is_headphone_connected(codec_number, pin_headphone_node_number)) {
            ...
            // Attempt to disable speaker?
            hda_send_verb(codec_number, pin_speaker_default_node_number, 0x707, (hda_send_verb(codec_number, pin_speaker_default_node_number, 0xF07, 0x00) & ~0x40));
        } else {
            printf("Headphones not connected.\n");
        }
    }
...
void hda_init_output_pin(uint32_t codec_number, uint32_t pin_node_number) {
    ...
    hda_set_node_gain(codec_number, pin_node_number, HDA_OUTPUT_NODE, pin_output_amp_capabilities, volume);

    uint32_t length_of_node_path = 0;
    hda_audio_mixer_depth = 0;
    ...
    if (type_of_first_connected_node == HDA_WIDGET_AUDIO_OUTPUT) {
        ...
        printf("Widget is audio mixer\n");
        hda_init_audio_mixer(codec_number, first_connected_node_number);
    } else if (type_of_first_connected_node == HDA_WIDGET_AUDIO_SELECTOR) {
        printf("Widget is audio selector\n");
    } else {
        printf("Widget is unknown\n");
    }
}

void hda_init_audio_mixer(uint32_t codec, uint32_t node) {
    hda_audio_mixer_depth++;
    if (hda_audio_mixer_depth >= 10) {
        printf("fuck.\n");
        return;
    }

    // Turn on power for the audio mixer
    hda_send_verb(codec, node, 0x705, 0x00);

    // Disable unsolicited responses
    hda_send_verb(codec, node, 0x708, 0x00);

    // Set volume for audio mixer
    uint32_t audio_mixer_amp_capabilities = hda_send_verb(codec, node, 0xF00, 0x12);
    hda_set_node_gain(codec, node, HDA_OUTPUT_NODE, audio_mixer_amp_capabilities, 40);
    ...
    uint32_t first_connected_node_number = hda_get_node_connection_entry(codec, node, 0); // get first node number
    uint32_t type_of_first_connected_node = hda_get_node_type(codec, node);           // get type of first node
    if (type_of_first_connected_node == HDA_WIDGET_AUDIO_OUTPUT)
    {
        printf("Found audio output inside audio mixer\n");
        hda_init_audio_output(codec, node);
    } else if (type_of_first_connected_node == HDA_WIDGET_AUDIO_MIXER) {
        printf("Found audio mixer inside audio mixer\n");
        hda_init_audio_mixer(codec, node);
    } else if (type_of_first_connected_node == HDA_WIDGET_AUDIO_SELECTOR) {
        printf("Found audio selector inside audio mixer\n");
    } else {
        printf("Found something else inside audio mixer");
    }
}
...
void hda_init_audio_output(uint32_t codec_number, uint32_t audio_output_node_number) {
    ...
    hda_set_node_gain(codec_number, audio_output_node_number, HDA_OUTPUT_NODE, audio_output_amp_capabilities, volume); // 100
...
void hda_play_pcm_data(uintptr_t addr) {
    ...
    hda_mmio_out32(hda_output_stream_base + 0x12, hda_return_sound_data_format(rate, 2, 16));

    // set Audio Output node data format
    hda_send_verb(0, aonn, 0x200, hda_return_sound_data_format(rate, 2, 16)); //15?
    for (size_t i = 0; i < 100000; i++) { }
